...
def stageParam(): #hardcoded Saturn V
  stages = 3
  dryMass     = [730000, 155600, 15200] #kg
  wetMass     = [2899000, 599000, 120500] #kg
  isp         = [304, 424, 424] #specific impulse by weight (seconds)
...
if GRAPH:
  ...
  plt.title("Rocket Performance: Payload vs. Δv, Saturn V")
  plt.legend(loc='best')
  plt.grid(True)
  plt.tight_layout()
  ...
  dv0 = dvs[0]
  peq  = max(payloads) / np.abs(initialSlope)
  npeq = dv0 / np.abs(initialSlope)
  payloadFraction = max(payloads) / max(wetMass)
  mipeq = -np.log10((normalisedEq) / (max(wetMass) * (payloadFraction **(3))))
  ...
  plt.title("Δv Sensitivity to Payload, Saturn V")
